...
function zt(t, n, s) {
  ...
  for (let v = 0; v < e; ++v) {
    if ((x.sort(A), s.history)) {
      ...
      (p.sort((f, O) => f.id - O.id),
        s.history.push({ x: x[0].slice(), fx: x[0].fx, simplex: p }));
    }
    d = 0;
    ...
    const l = x[b];
    if ((Q(S, 1 + h, T, -h, l), (S.fx = t(S)), S.fx < x[0].fx))
      (Q(m, 1 + u, T, -u, l), (m.fx = t(m)), m.fx < S.fx ? M(m) : M(S));
      ...
      for (let u = 0; u < h.sets.length; u++) {
        ...
        for (let y = u + 1; y < h.sets.length; y++) {
          const d = String(h.sets[y]),
            b = `${r};${d}`,
            x = `${d};${r}`;
          (c.set(b, h.size + (c.get(b) || 0)),
            c.set(x, h.size + (c.get(x) || 0)));
        }
      }
  ...
  return (
    t
      ...
      .forEach((o) => {
        ...
        e[a][c] = e[c][a] = r;
        let y = 0;
        (o.size + 1e-10 >= Math.min(n[a].size, n[c].size)
          ? (y = 1)
          : o.size <= 1e-10 && (y = -1),
          (i[a][c] = i[c][a] = y));
      }),
...
function pe(t, n, s) {
  if (
    (s == null ? t.sort((i, o) => o.radius - i.radius) : t.sort(s),
    t.length > 0)
  ) {
    const i = t[0].x,
      o = t[0].y;
...
function dt(t) {
  const n = (s) => {
    const e = t.reduce(
        (o, a) => Math.max(o, a[s] + a.radius),
        Number.NEGATIVE_INFINITY,
      ),
      i = t.reduce(
        (o, a) => Math.min(o, a[s] - a.radius),
...
function Dt(t, n, s) {
  ...
  for (const u of i) {
    pe(u, n, s);
    const r = dt(u);
    ((u.size = (r.xRange.max - r.xRange.min) * (r.yRange.max - r.yRange.min)),
      (u.bounds = r));
  }
  ...
  function c(u, r, y) {
    ...
    if (r) b = o.xRange.max - d.xRange.min + a;
      ...
      const M =
        (d.xRange.max - d.xRange.min) / 2 - (o.xRange.max - o.xRange.min) / 2;
      M < 0 && (b += M);
    }
    if (y) x = o.yRange.max - d.yRange.min + a;
      ...
      const M =
        (d.yRange.max - d.yRange.min) / 2 - (o.yRange.max - o.yRange.min) / 2;
      M < 0 && (x += M);
    }
...
function be(t = {}) {
  ...
  function l(p) {
    ...
    const O = new Set();
    (f.forEach((k) => {
      k.size == 0 && k.sets.length == 1 && O.add(k.sets[0]);
    }),
      (f = f.filter((k) => !k.sets.some((F) => O.has(F)))));
    let I = {},
      C = {};
    if (f.length > 0) {
      let k = m(f, { lossFunction: v, distinct: b });
      (c && (k = Dt(k, a, d)), (I = Ot(k, s, e, i, h)), (C = jt(I, f, M)));
    }
    const U = {};
    f.forEach((k) => {
    ...
    n
      ? E.attr("viewBox", `0 0 ${s} ${e}`)
      : E.attr("width", s).attr("height", e);
    const R = {};
    let _ = !1;
    ...
    const B = E.selectAll(".venn-area").data(f, (k) => k.sets),
      N = B.enter()
        .append("g")
        .attr(
          "class",
    ...
    return (
      y !== null &&
        ...
        L.style("font-size", y),
        X.style("font-size", "0px")),
      { circles: I, textCentres: C, nodes: B, enter: N, update: z, exit: D }
    );
  }
  return (
    (l.wrap = function (p) {
    ...
    (l.lossFunction = function (p) {
      return arguments.length
        ? ((v = p === "default" ? tt : p === "logRatio" ? Ct : p), l)
        : v;
    }),
    l
  );
...
var gt = (function () {
  var t = w(function (S, g, m, v) {
    ...
    M = {
      ...
      performAction: w(function (g, m, v, l, p, f, O) {
        var I = f.length - 1;
        switch (p) {
          case 1:
            return f[I - 1];
          ...
          switch (D[0]) {
            ...
            case 2:
              ...
              (H &&
                ...
                p.push(k.$),
                f.push(k._$),
                (et = O[v[v.length - 2]][v[v.length - 1]]),
                v.push(et));
              break;
            case 3:
              return !0;
      ...
      var S = {
        ...
        unput: w(function (g) {
          ...
          return (
            ...
            this.options.ranges &&
              (this.yylloc.range = [p[0], p[0] + this.yyleng - m]),
...
` +
        ...
        next: w(function () {
          ...
          for (var p = this._currentRules(), f = 0; f < p.length; f++)
            if (
              ((v = this._input.match(this.rules[p[f]])),
...
var Ve = w((t, n, s, e) => {
  ...
  const l = ot(document.createElement("div")),
    p = be()
      ...
      .height(A - g);
  l.datum(d).call(p);
  const f = u ? Jt.svg(l.select("svg").node()) : void 0,
    O = Me(d, { width: M, height: A - g, padding: o?.padding ?? 15 }),
    I = new Map();
  for (const E of O) {
    const R = J([...E.data.sets].sort());
    I.set(R, E);
  }
  ...
  (l.selectAll(".venn-circle").each(function (E, R) {
    const _ = ot(this),
      B = J([...E.sets].sort()),
      N = x.get(B),
      V = N?.fill || r[R % r.length] || a.primaryColor;
    _.classed(`venn-set-${R % 8}`, !0);
    const W = N?.["fill-opacity"] ?? 0.1,
      K = N?.stroke || V,
      z = N?.["stroke-width"] || `${5 * S}`;
    if (u && f) {
      const D = I.get(B);
      if (D && D.circles.length > 0) {
        ...
        (_.select("path").remove(),
          _.node()?.insertBefore(k, _.select("text").node()));
      }
    } else
      _.select("path")
        .style("fill", V)
    ...
    const L = N?.color || (C ? te(V, 30) : ee(V, 30));
    _.select("text")
          ...
          if (N) {
            ...
            if (W) {
              const K = f.path(W, {
                ...
                z = V.node();
              (z?.parentNode?.insertBefore(K, z), V.remove());
            }
          } else R.select("path").style("fill-opacity", 0);
          R.select("text")
            .style("font-size", `${48 * S}px`)
            .style("fill", B?.color ?? a.vennSetTextColor ?? m);
        })
      : (l
          .selectAll(".venn-intersection text")
          .style("font-size", `${48 * S}px`)
          .style("fill", (E) => {
            const _ = J([...E.sets].sort());
  ...
  if (j && "childNodes" in j)
    for (const E of [...j.childNodes]) U.node()?.appendChild(E);
...
var qe = { draw: Ve },
  Ze = { parser: Se, db: je, renderer: qe, styles: Be };
