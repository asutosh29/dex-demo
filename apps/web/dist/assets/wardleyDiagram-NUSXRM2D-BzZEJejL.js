...
import { p as Bt } from "./chunk-4BX2VUAB-BT_kZFii.js";
import { p as Rt } from "./wardley-RL74JXVD-CKQuThhO.js";
...
var H = y((a, r) => {
  ...
  E = y(
    (a, r, e) => ({ x: H(r, `${e} evolution`), y: H(a, `${e} visibility`) }),
    "toCoordinates",
  ),
  K = y((a) => {
    if (a) {
  ...
  Ot = y((a, r) => {
    if (
      (Bt(a, r), a.size && r.setSize(a.size.width, a.size.height), a.evolution)
    ) {
      const e = a.evolution.stages.map((s) =>
          s.secondName
            ? `${s.name.trim()} / ${s.secondName.trim()}`
            : s.name.trim(),
        ),
        x = a.evolution.stages
          .filter((s) => s.boundary !== void 0)
          .map((s) => s.boundary);
    ...
    if (
      ...
      a.components.forEach((e) => {
        const x = E(e.visibility, e.evolution, `Component "${e.name}"`),
          s = e.label ? (e.label.negX ? -1 : 1) * e.label.offsetX : void 0,
          d = e.label ? (e.label.negY ? -1 : 1) * e.label.offsetY : void 0,
          m = e.decorator?.strategy;
        r.addNode(e.name, e.name, x.x, x.y, "component", s, d, e.inertia, m);
      }),
      a.notes.forEach((e) => {
        ...
        (r.startPipeline(e.parent),
          e.components.forEach((d) => {
            const m = `${e.parent}_${d.name}`,
              M = d.label ? (d.label.negX ? -1 : 1) * d.label.offsetX : void 0,
              g = d.label ? (d.label.negY ? -1 : 1) * d.label.offsetY : void 0,
              I = H(d.evolution, `Pipeline component "${d.name}" evolution`);
            (r.addNode(m, d.name, I, s, "pipeline-component", M, g),
      ...
      a.evolves.forEach((e) => {
        ...
        if (x?.y !== void 0) {
          const s = H(e.target, `Evolve target for "${e.component}"`);
    ...
    (a.annotation.forEach((e) => {
      ...
      a.deaccelerators.forEach((e) => {
        const x = E(e.x, e.y, `Deaccelerator "${e.name}"`);
...
y(ot, "addTrend");
function st(a, r, e) {
  P.addAnnotation({ number: a, coordinates: r, text: e ? $(e) : void 0 });
}
y(st, "addAnnotation");
function nt(a, r, e) {
  P.addNote({ text: $(a), x: r, y: e });
}
y(nt, "addNote");
function it(a, r, e) {
  P.addAccelerator({ name: $(a), x: r, y: e });
}
y(it, "addAccelerator");
function dt(a, r, e) {
  P.addDeaccelerator({ name: $(a), x: r, y: e });
}
...
function ft(a) {
  const r = {};
  (a.xLabel && (r.xLabel = $(a.xLabel)),
    a.yLabel && (r.yLabel = $(a.yLabel)),
    a.stages && (r.stages = a.stages.map((e) => $(e))),
...
var Wt = {
  ...
  Dt = ["Genesis", "Custom Built", "Product", "Commodity"],
  Gt = y(() => {
    const { themeVariables: a } = q();
    return {
      ...
      componentLabelColor:
        a.wardley?.componentLabelColor ?? a.primaryTextColor ?? "#222",
      ...
      annotationTextColor:
        a.wardley?.annotationTextColor ?? a.primaryTextColor ?? "#222",
      annotationFill: a.wardley?.annotationFill ?? a.background ?? "#fff",
    };
  }, "getTheme"),
  Ht = y(() => {
    const a = q()["wardley-beta"];
    return {
      width: a?.width ?? 900,
      height: a?.height ?? 600,
...
` + a,
    ...
    const s = Ht(),
      ...
      g = M.getWardleyData(),
      I = M.getDiagramTitle(),
      C = g.size?.width ?? s.width,
      b = g.size?.height ?? s.height,
      B = At(r);
    (B.selectAll("*").remove(),
      Et(B, b, C, s.useMaxWidth),
      B.attr("viewBox", `0 0 ${C} ${b}`));
    const v = B.append("g").attr("class", "wardley-map"),
      j = B.append("defs");
    (j
      .append("marker")
      .attr("id", `arrow-${r}`)
    ...
    I &&
      v
        ...
        .attr("font-size", s.axisFontSize * 1.05)
        .attr("font-weight", "bold")
        .attr("text-anchor", "middle")
        .attr("dominant-baseline", "middle")
        .text(I);
    const z = y((t) => s.padding + (t / 100) * R, "projectX"),
      X = y((t) => b - s.padding - (t / 100) * F, "projectY"),
      W = v.append("g").attr("class", "wardley-axes");
    (W.append("line")
      .attr("x1", s.padding)
      .attr("x2", C - s.padding)
      .attr("y1", b - s.padding)
    ...
    (W.append("text")
      ...
      W.append("text")
        ...
        .attr("x", s.padding / 3)
        .attr("y", s.padding + F / 2)
        .attr("fill", d.axisTextColor)
        .attr("font-size", s.axisFontSize)
        .attr("font-weight", "bold")
        .attr("text-anchor", "middle")
        .attr("transform", `rotate(-90 ${s.padding / 3} ${s.padding + F / 2})`)
        .text(mt));
    const O = g.axes.stages && g.axes.stages.length > 0 ? g.axes.stages : Dt;
    if (O.length > 0) {
      ...
      for (let n = 1; n < 4; n++) {
        ...
        (t
          ...
          t
            ...
            .attr("x1", s.padding)
            .attr("x2", C - s.padding)
            .attr("y1", b - s.padding - F * o)
            .attr("y2", b - s.padding - F * o)
            .attr("stroke", d.gridColor)
            .attr("stroke-dasharray", "2 6"));
      }
    }
    ...
    if (
      (g.nodes.forEach((t) => {
        c.set(t.id, { x: z(t.x), y: X(t.y), node: t });
      }),
      g.pipelines.length > 0)
    ) {
      const t = v.append("g").attr("class", "wardley-pipelines"),
        n = v.append("g").attr("class", "wardley-pipeline-links");
      g.pipelines.forEach((o) => {
        if (o.componentIds.length === 0) return;
        const i = o.componentIds
          .map((f) => ({
            id: f,
            pos: c.get(f),
            node: g.nodes.find((u) => u.id === f),
          }))
        ...
        let p = 1 / 0,
          ...
          h = 0;
        if (
          (o.componentIds.forEach((f) => {
            const u = c.get(f);
            u && ((p = Math.min(p, u.x)), (l = Math.max(l, u.x)), (h = u.y));
          }),
          p !== 1 / 0 && l !== -1 / 0)
        ) {
          const u = s.nodeRadius * 4,
            w = h - u / 2,
            S = c.get(o.nodeId);
          if (S) {
            const L = (p + l) / 2;
    ...
    const Z = v.append("g").attr("class", "wardley-links"),
      U = new Map();
    g.pipelines.forEach((t) => {
      U.set(t.nodeId, new Set(t.componentIds));
    });
    const J = g.links.filter(
      (t) =>
        !(
          !c.has(t.source) ||
          !c.has(t.target) ||
          U.get(t.target)?.has(t.source)
        ),
    );
    (Z.selectAll("line")
      .data(J)
      ...
      Z.selectAll("text")
        ...
        .attr("transform", (t) => {
          ...
          return ((T > 90 || T < -90) && (T += 180), `rotate(${T} ${L} ${D})`);
        })
        .text((t) => t.label));
    const wt = v.append("g").attr("class", "wardley-trends"),
      bt = g.trends
        .map((t) => {
    ...
    (N.filter((t) => t.sourceStrategy === "outsource")
      ...
      N.filter((t) => t.sourceStrategy === "buy")
        ...
        .attr("stroke", d.componentStroke)
        .attr("stroke-width", 1),
      N.filter((t) => t.sourceStrategy === "build")
        .append("circle")
    ...
    if (
      ...
      A.append("circle")
        ...
        .attr("stroke", d.componentStroke)
        .attr("stroke-width", 2),
      N.filter((t) => t.isPipelineParent === !0)
        .append("rect")
      ...
      N.filter((t) => t.inertia === !0)
        ...
        .attr("y1", (t) => {
          const n = c.get(t.id),
            o = t.isPipelineParent ? m : s.nodeRadius * 2;
          return n.y - o / 2;
        })
        .attr("x2", (t) => {
          const n = c.get(t.id);
      ...
      if (
        (g.annotations.forEach((n) => {
          const o = n.coordinates.map((i) => ({ x: z(i.x), y: X(i.y) }));
          if (o.length > 1)
        ...
        let n = z(g.annotationsBox.x),
          o = X(g.annotationsBox.y);
        const i = 10,
          p = 16,
          l = 11,
        ...
        if (
          ...
          u.forEach((V) => {
            const G = V.node(),
              St = G.getComputedTextLength();
            w = Math.max(w, St);
            const $t = G.getBBox();
            S = Math.max(S, $t.height);
          });
          const L = w + i * 2 + 105,
            D = f.length * p + i * 2 + S / 2,
            T = s.padding,
            kt = C - s.padding - L,
            Pt = s.padding,
            vt = b - s.padding - D;
          ((n = Math.max(T, Math.min(n, kt))),
            (o = Math.max(Pt, Math.min(o, vt))),
            u.forEach((V, G) => {
  ...
  jt = { draw: qt },
  se = { parser: tt, db: Wt, renderer: jt, styles: y(() => "", "styles") };
