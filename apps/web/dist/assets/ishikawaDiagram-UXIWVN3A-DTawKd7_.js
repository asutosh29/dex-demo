      ...
      var T = {
        ...
        unput: o(function (t) {
          ...
          return (
            ...
            this.options.ranges &&
              (this.yylloc.range = [f[0], f[0] + this.yyleng - s]),
...
` +
        ...
        next: o(function () {
          ...
          for (var f = this._currentRules(), n = 0; n < f.length; n++)
            if (
              ((i = this._input.match(this.rules[f[n]])),
...
var xt = J,
  ...
  It = o((e, h, r, a) => {
    ...
    let i = 0,
      c = j;
    const f = g ? void 0 : z(E, i, c, i, c, "ishikawa-spine");
    if ((Lt(E, i, c, y.text, k, t), !_.length)) {
      (g && z(E, i, c, i, c, "ishikawa-spine", t), at(O, x, D));
      return;
    }
    i -= 20;
    const n = _.filter((S, B) => B % 2 === 0),
      v = _.filter((S, B) => B % 2 === 1),
      w = rt(n),
      I = rt(v),
      $ = w.total + I.total;
    let L = j,
    ...
    if ($ > 0) {
      const S = j * 2,
        B = j * 0.3;
      ((L = Math.max(B, S * (w.total / $))),
        (A = Math.max(B, S * (I.total / $))));
    }
    const C = k * 2;
    ...
    for (let S = 0; S < b; S++) {
      const B = E.append("g").attr("class", "ishikawa-pair");
      for (const [R, G, Y] of [
        [_[S * 2], -1, L],
        [_[S * 2 + 1], 1, A],
      ])
        R && Bt(B, R, i, c, G, Y, k, t);
      i = B.selectAll("text")
        .nodes()
    ...
    if (g) z(E, i, c, 0, c, "ishikawa-spine", t);
      ...
      f.attr("x1", i);
      const S = `url(#${s})`;
      E.selectAll("line.ishikawa-branch, line.ishikawa-sub-branch").attr(
        "marker-start",
        S,
      );
    }
  ...
  Lt = o((e, h, r, a, l, y) => {
    const p = Math.max(6, Math.floor(110 / (l * 0.6))),
      ...
      m = X(d, ct(a, p), 0, 0, "ishikawa-head-label", "start", l),
      u = m.node().getBBox(),
    ...
    m.attr(
      "transform",
      `translate(${(k - u.width) / 2 - u.x + 3},${-u.y - u.height / 2})`,
    );
  }, "drawHead"),
  Tt = o((e, h) => {
    const r = [],
    ...
    for (const [f, n] of D.entries()) {
      ...
      if (n.depth % 2 === 0) {
        ...
        (($ = lt(w.x0, w.x1, C ? (v - w.y0) / C : 0.5)),
          ...
          d && Q(I, $, v, 1, 0, d),
          X(I, n.text, A, v, "ishikawa-label align", "end", p));
      } else {
